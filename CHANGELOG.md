...
- **BREAKING — secret key format.** `RefCampaignServer` now requires keys
  starting with `rc_live_` or `rc_test_`, matching the platform's opaque-token
  format (refs platform #679). The previous `sk_` prefix is rejected. Old `sk_`
  keys no longer exist — the platform purged them when it migrated to opaque
  tokens, so any key issued from the dashboard already uses the new format.
  Update your `new RefCampaignServer(secretKey)` call sites with the key copied
